...
**New minor finding (cosmetic only, not accounting-correctness):** the one seeded account whose name contains an ampersand — `4200 Sales Returns & Allowances` — renders as **"Sales Returns &amp;amp; Allowances"** on both the Trial Balance and Budget vs Actual pages (visible in the screenshots above). Traced the full pipeline to rule out the obvious suspects: the DB stores a clean single `&` (confirmed via `DB::table('account')->where('code','4200')->first()->name`), the shared `decodeTranslations()` helper in both `GeneralLedgerService.php` and `BudgetVarianceReportService.php` does a plain `json_decode()` with no escaping, and the frontend slot renderers (`getLocalizedName()` in `accountingHelpers.ts`, used by `TrialBalance.tsx`) render the value as plain React text with no `dangerouslySetInnerHTML` anywhere in the codebase (confirmed via a repo-wide grep — zero matches). By elimination, this points to Yajra DataTables' **default column-escaping** (`escapeColumns`), which HTML-escapes every column not explicitly opted out via `rawColumns()` — including recursing into the nested `en`/`ar` keys of an already-decoded translation array — and neither `GeneralLedgerService::trialBalanceDataTable()` nor `BudgetVarianceReportService::datatable()` (nor, most likely, the ~10 other report services sharing this same `decodeTranslations()`-then-`editColumn()` pattern found via `grep -l decodeTranslations app/Application/Reports`) calls `rawColumns()` for their translated-name columns.

**Not fixed** — this is purely a display artifact (one seeded account name happens to contain "&"; no amounts, balances, or postings are affected) and the real fix is a `rawColumns([...])` addition across a double-digit number of report/DataTable services, which is a broader, multi-file change outside today's scope of *fixing what today's testing broke or found broken in core posting logic*. Flagging for a decision on whether to schedule it, the same way the VAT double-counting finding in §12.2 was flagged rather than unilaterally fixed.

### 12.12 What's next

Only 11.10 remains (the commit decision — purely your call, nothing left to test). Every §11 item is now closed:

- **11.1–11.5, 11.7**: executed against real data, zero defects except two real bugs found and fixed (VAT filing schema bug §12.2, `BudgetVarianceReportService` json-GROUP BY bug §12.6).
- **11.6** (multi-currency): verified out of scope, not untested — the feature doesn't exist yet (§12.8).
- **11.8** (visual UI pass): done — the DataTable slot fix confirmed live across 9 report pages with zero console errors, plus one new minor cosmetic finding flagged, not fixed (§12.11).
...
Every core module (Purchasing, Sales, Rentals, Payroll, Fixed Assets, Cheques, VAT, Inventory, Period Close, Bank Reconciliation, Prepaid/Accrued, Projects/Budgets) has now been exercised end-to-end with real data through the actual application service layer. Total defects found and fixed across the full session: **7** (2 responsive/pagination bug classes, 2 `PeriodService` column-name typos, 1 `tax_returns`/`tax_periods` UUID-vs-bigint schema bug, the 11-file DataTable slot-targeting bug, and the `BudgetVarianceReportService` json-GROUP BY bug) — all regression-tested on both Postgres (real data) and SQLite (PHPUnit). Two findings were surfaced and deliberately **not** fixed as unilateral code changes, since both are product/scope decisions: the VAT double-counting edge case (§12.2) and the Yajra `rawColumns()` HTML-entity-escaping cosmetic issue (§12.11).
