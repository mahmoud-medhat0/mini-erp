...
GUIDE_VERSION = "1.3"
GUIDE_DATE_ISO = "2026-09-22"
GUIDE_DATE_AR = "22 سبتمبر 2026"
...
MODULES: list[Module] = [
    Module(
        ...
        [
            ...
            Screen(
                "/accounting/coa",
                "دليل الحسابات",
                "عرض شجرة الحسابات وإنشاء المجموعات والحسابات وضبط العملة وحسابات المراقبة. البحث بالكود أو الاسم، والتصفية حسب المجموعة والنوع والعملة والطبيعة ونمط الحساب والحالة.",
            ),
    ...
    Module(
        ...
        [
            ...
            Screen("/sales/quotations", "عروض الأسعار", "عرض سعر للعميل قبل الالتزام، بحالات مسودة وإرسال وقبول أو رفض، وتحويل العرض المقبول إلى أمر بيع."),
        ...
        ["عميل", "عرض سعر", "أمر بيع", "إذن تسليم", "فاتورة", "قبض وتخصيص"],
        [
            "عرض السعر لا يُنشئ أثرًا محاسبيًا؛ الأثر يبدأ من أمر البيع المحوَّل منه أو المُنشأ مباشرة.",
    ...
    Module(
        ...
        [
            Screen("/purchasing/requests", "طلبات الشراء", "طلب شراء داخلي يمر باعتماد قبل تحويله لأمر شراء فعلي، لضبط من يطلب الشراء قبل الالتزام بمورد."),
        ...
        ["طلب شراء", "مورد", "أمر شراء", "استلام", "تكلفة وصول", "فاتورة ودفع"],
        [
            "طلب الشراء لا يُنشئ التزامًا على مورد؛ الالتزام يبدأ من أمر الشراء.",
    ...
    Module(
        "العُدّة والمعدات",
        "عهدة داخلية للأدوات والمعدات المستخدمة في تشغيل الشركة نفسها، منفصلة عن المخزون المعد للبيع وعن الإيجار للعملاء الخارجيين.",
        [
            Screen("/equipment/categories", "فئات العُدّة", "تصنيف العُدّة والمعدات قبل تسجيلها في السجل الرئيسي."),
            Screen(
                "/equipment/tools",
                "سجل العُدّة والمعدات",
                "تسجيل الصنف والرقم التسلسلي إن وُجد والحالة، وتسليمها لموظف أو فرع، ونقلها بين المواقع، وتسجيلها تالفة أو مفقودة أو تحت الصيانة.",
            ),
        ],
        ["تسجيل صنف", "تسليم لعهدة", "نقل بين المواقع", "إرجاع للمخزن", "تسجيل حالة استثنائية"],
        [
            "العُدّة عهدة داخلية بلا مقابل مالي من طرف ثالث؛ الإيجار للعميل الخارجي وحدة مستقلة عنها.",
            "لا يوجد أثر محاسبي إلزامي عند التسليم أو النقل أو تغيير الحالة.",
            "حالات الصنف: متاح، مُسلَّم، تالف، مفقود، تحت الصيانة، مستبعد.",
        ],
    ),
    Module(
    ...
    Module(
        ...
        "تسجيل المصروفات المباشرة وجدولة المصروف المقدم والمستحق وترحيل الاعتراف الدوري، مع محرك مستقل لتوليد مسودة مصروف متكرر تلقائيًا حسب جدول زمني.",
        [
            ...
            Screen(
                "/recurring/templates",
                "القوالب المتكررة",
                "إعداد مصروف متكرر بجدول زمني أسبوعي أو شهري أو ربع سنوي أو سنوي، لإنشاء مسودة مصروف تلقائيًا في كل موعد استحقاق تنتظر مراجعة المستخدم واعتماده.",
            ),
        ],
        ...
        [
            ...
            "القالب المتكرر يُنشئ مسودة مصروف فقط عند كل استحقاق، ولا يرحلها تلقائيًا؛ الاعتماد والترحيل يبقيان بيد المستخدم.",
        ],
    ),
    Module(
        "المرتبات",
        ...
        [
            ...
            Screen(
                "/payroll/loans",
                "سلف وقروض الموظفين",
                "تسجيل سلفة أو قرض لموظف بصرف فعلي، ثم خصم القسط تلقائيًا من كل كشف مرتب لاحق حتى اكتمال السداد.",
            ),
        ],
        ...
        [
            ...
            "قسط السلفة أو القرض يُخصم تلقائيًا من كل كشف حتى اكتمال الرصيد المتبقي؛ لا يحتاج إدخالًا يدويًا كل فترة.",
        ],
    ),
    Module(
        "الإيجارات",
    ...
    Module(
        ...
        [
            ...
            Screen(
                ...
                "تسجيل استقطاع ضريبي يدوي على دفعة لمورد باستخدام كود ونسبة مُعدّة مسبقًا، ثم ترحيله لحساب التزام مستقل عن حساب الموردين.",
            ),
        ],
        ...
        [
            ...
            "إطار ضريبة الخصم والإضافة تقني بحت؛ لا يحدد النظام متى تُستحق الضريبة قانونًا ولا نسبتها الفعلية، وهذه تحتاج مراجعة مستشار ضريبي قبل استخدامها في أي إقرار رسمي.",
        ],
    ),
    Module(
    ...
    Module(
        ...
        "سجل الشركاء ونسبة حصة كل شريك، مع مساهماتهم ومسحوباتهم وتوزيعات الأرباح وقروضهم للشركة، مرتبطة بحسابات مراقبة مشتركة في دليل الحسابات.",
        [
            Screen("/partners", "الشركاء", "بيانات كل شريك وكوده ونسبة حصته وحالته."),
            Screen(
                "/partners/transactions",
                "معاملات الشركاء",
                "تسجيل مساهمة أو سحب أو توزيع أرباح كمسودة بمبلغ وتاريخ، ثم ترحيلها بتحديد حساب التسوية النقدي أو البنكي.",
            ),
            Screen(
                "/partners/loans",
                "قروض الشركاء",
                "تسجيل قرض من شريك للشركة بصرف فعلي، ثم تسجيل كل دفعة سداد له على حدة حتى اكتمال الرصيد المتبقي.",
            ),
        ],
        ["شريك", "مساهمة أو سحب أو توزيع", "ترحيل", "قرض شريك", "سداد"],
        [
            "المساهمة تزيد رصيد حساب رأس مال الشركاء المشترك، والسحب والتوزيع يخفضانه.",
            "قرض الشريك عكس سلفة الموظف: الشركة تستلم نقدية من الشريك وتُنشئ التزامًا تجاهه.",
            "الترحيل يحتاج تحديد الحساب النقدي أو البنكي الذي تمت به التسوية الفعلية.",
            "لا يوجد ترحيل تلقائي دوري لأرباح الفترة إلى حساب رأس مال الشركاء؛ التوزيع يُسجَّل يدويًا كمعاملة مستقلة.",
        ],
    ),
]
...
ALL_ROUTE_GROUPS: list[tuple[str, list[tuple[str, str]]]] = [
    ...
    (
        "العُدّة والمعدات والشركاء وحقوق الملكية",
        [
            ("/equipment/categories", "فئات العُدّة"),
            ("/equipment/tools", "سجل العُدّة والمعدات"),
            ("/partners", "الشركاء"),
            ("/partners/transactions", "معاملات الشركاء"),
            ("/partners/loans", "قروض الشركاء"),
        ],
    ),
    (
        "الأصول الثابتة",
    ...
    (
        ...
        [
            ...
            ("/taxes/withholding", "ضريبة الخصم والإضافة"),
            ("/projects", "المشروعات"),
    ...
    (
        ...
        [
            ...
            ("/reports/payroll", "تقرير المرتبات"),
            ...
            ("/reports/forecast", "التنبؤ المالي"),
            ("/reports/equity-statement", "قائمة التغير في حقوق الملكية"),
            ("/reports/reorder-level", "الأصناف تحت حد إعادة الطلب"),
...
def build_story() -> list[Flowable]:
    ...
    assert route_count == 149, f"Expected 149 active screens, found {route_count}."
