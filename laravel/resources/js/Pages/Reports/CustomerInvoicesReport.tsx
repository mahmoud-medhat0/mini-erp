...
interface CustomerInvoicesReportProps {
    reportData: {
...
export default function CustomerInvoicesReport({ reportData, filters, customers, products }: CustomerInvoicesReportProps) {
    ...
    return (
        <AppLayout active="reports.customer-invoices">
            <Head title="Customer Invoices Report / تقرير فواتير العملاء" />

            <div className="py-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
                <PageHeader
                    title="Customer Invoices Register / سجل فواتير العملاء"
                    description="Read-only operational register of all customer invoices with accounting links"
                />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Card className="p-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                        <div className="text-xs font-medium text-slate-500">Total Customer Invoices / إجمالي فواتير العملاء</div>
                        <div className="text-2xl font-bold text-slate-900 dark:text-slate-100 mt-1">{reportData.summary.total_invoices_count}</div>
                    </Card>
                    <Card className="p-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                        <div className="text-xs font-medium text-slate-500">Total Invoiced Amount / إجمالي المبلغ المفوتر</div>
                    ...
                    <div className="overflow-x-auto">
                        <table className={tableClasses.table}>
                            <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
                                <tr>
                                    <th className={tableClasses.th}>Invoice # / رقم الفاتورة</th>
                                    ...
                                    <th className={tableClasses.th}>AR Entry / قيد الذمم</th>
                                </tr>
                            </thead>
                                ...
                                {reportData.rows.length === 0 ? (
                                    <tr>
                                        <td colSpan={8} className="p-4 text-center">
                                            <EmptyState title="No customer invoices found / لا توجد فواتير عملاء" />
                                        </td>
                                    </tr>
                                ) : (
