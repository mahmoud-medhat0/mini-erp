...
export default function FixedAssetRegisterReport({ locale, assets, filters }: ReportProps) {
  ...
  function applyFilters() {
    router.get('/reports/fixed-asset-register', { search, status }, { preserveState: true, preserveScroll: true, replace: true });
  }
  ...
  function exportHref() {
    ...
    return `/reports/fixed-asset-register/export${query ? `?${query}` : ''}`;
  }
  ...
  return (
    <AppLayout active="reports.index">
      <Head title={reportDict.fixedAssetRegisterReport} />

      <PageHeader
        title={reportDict.fixedAssetRegisterReport}
        description={reportDict.fixedAssetRegisterReportDescription}
        actions={
          <>
            {canExport ? (
