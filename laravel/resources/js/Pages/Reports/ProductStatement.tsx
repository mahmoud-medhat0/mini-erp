...
type ProductStatementProps = SharedPageProps & {
  report: {
    entity: { id: string; code: string; name: TranslatedName; uom_code?: string | null };
    filters: { product_id: string | null; warehouse_id: string | null; date_from: string; date_to: string; currency: string };
    opening_balance_quantity_e6: number;
    opening_balance_value_minor: number;
    total_in_quantity_e6: number;
    total_out_quantity_e6: number;
    total_in_value_minor: number;
    total_out_value_minor: number;
    closing_balance_quantity_e6: number;
...
export default function ProductStatement({ locale, report, products, warehouses, currencies, filters }: ProductStatementProps) {
  const dict = getDictionary(locale);
  const pageDict = dict.app.pages.reportsProductStatement;
  ...
  const uomCode = report?.entity.uom_code || '';
  ...
  const tableColumns = useMemo(() => [
    ...
    { data: 'warehouse_name', name: 'warehouse_name', title: pageDict.warehouseColumn, orderable: false },
  ...
  const tableSlots = useMemo<StatementTableSlots>(() => ({
    ...
    // warehouse_name/warehouse_code arrive already locale-resolved from the backend
    // (a window-function query, not an Eloquent model), so no getLocalizedName() here.
    warehouse_name: (data, row) => (
      <span>
        {row.warehouse_code ? <strong>{row.warehouse_code}</strong> : null}
    ...
    quantity_delta_e6: (data) => {
      ...
      return (
        <span className={`font-mono font-bold ${value >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'}`}>
          {value >= 0 ? '+' : ''}{formatQuantityE6(value)} {uomCode}
        </span>
      );
    },
    ...
    balance_quantity_e6: (data) => <span className="font-mono font-bold">{formatQuantityE6(Number(data))} {uomCode}</span>,
  ...
  }), [report?.filters.currency, uomCode]);
  ...
  const hasActiveFilters = Boolean(productId || warehouseId || dateFrom || dateTo);

  const handleFilter = () => {
    router.get('/reports/product-statement', {
  ...
  const handleReset = () => {
    setProductId('');
    setWarehouseId('');
    router.get('/reports/product-statement', { currency }, { preserveScroll: true });
  };

  const handleExport = () => {
    if (!productId) return;
    const url = `/reports/product-statement/export?product_id=${productId}&warehouse_id=${warehouseId}&date_from=${dateFrom}&date_to=${dateTo}&currency=${currency}`;
    window.open(url, '_blank');
  };

  return (
    <AppLayout active="reports.product-statement">
      <Head title={pageDict.productStatementMiniErp} />

      <PageHeader
        title={pageDict.productStatement}
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            <div>
              ...
              <SearchableSelect
                ...
                placeholder={pageDict.selectProduct}
              />
            </div>
            <div>
              <label className="block text-xs font-semibold mb-1 text-[var(--text-secondary)]">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              <div className="bg-[var(--card)] p-3 rounded-lg border border-[var(--border-color)]">
                ...
                <div className="text-sm font-bold text-[var(--text-primary)]">{formatQuantityE6(report.opening_balance_quantity_e6)} {uomCode}</div>
              </div>
              <div className="bg-[var(--card)] p-3 rounded-lg border border-[var(--border-color)]">
              ...
              <div className="bg-[var(--card)] p-3 rounded-lg border border-[var(--border-color)]">
                ...
                <div className="text-sm font-bold text-emerald-600">+{formatQuantityE6(report.total_in_quantity_e6)} {uomCode}</div>
              </div>
              <div className="bg-[var(--card)] p-3 rounded-lg border border-[var(--border-color)]">
                <div className="text-xs text-[var(--text-secondary)] mb-1">{pageDict.totalOut}</div>
                <div className="text-sm font-bold text-amber-600">-{formatQuantityE6(report.total_out_quantity_e6)} {uomCode}</div>
              </div>
              <div className="bg-[var(--card)] p-3 rounded-lg border border-[var(--border-color)]">
                <div className="text-xs text-[var(--text-secondary)] mb-1">{pageDict.closingBalanceQuantity}</div>
                <div className="text-sm font-bold text-[var(--text-primary)]">{formatQuantityE6(report.closing_balance_quantity_e6)} {uomCode}</div>
              </div>
              <div className="bg-[var(--card)] p-3 rounded-lg border border-[var(--border-color)]">
            ...
            <Card className="overflow-hidden p-0">
              <div className="border-b border-[var(--border-color)] bg-[var(--background)]/50 px-4 py-3 text-xs font-bold">
                {pageDict.openingBalancePriorToRange}: {formatQuantityE6(report.opening_balance_quantity_e6)} {uomCode} / {formatMoney(report.opening_balance_value_minor, report.filters.currency)}
              </div>
              <ServerDataTable
                key={`${filters.product_id}-${filters.warehouse_id}-${filters.date_from}-${filters.date_to}-${filters.currency}`}
                ajaxUrl="/reports/product-statement/data"
                ...
                pageLength={25}
                slots={tableSlots}
                tableId="product-statement-data-table"
              />
            </Card>
          </div>
        ) : (
          <Card className="p-12 text-center text-[var(--text-muted)]">
            {pageDict.pleaseSelectAProductAndPeriod}
          </Card>
        )}
      </div>
    </AppLayout>
