...
export default function PayableSettlements({
  ...
  const columns = useMemo(() => [
    ...
    { data: 'source_payable_entry_id', name: 'source_payable_entry_id', title: pageDict.sourceEntry },
    { data: 'target_payable_entry_id', name: 'target_payable_entry_id', title: pageDict.targetEntry },
  ...
  const slots = useMemo<DataTableSlots>(() => ({
    ...
    supplier_name: (data: string | Record<string, string>): ReactElement => <span className="font-semibold">{getLocalizedName(data, locale)}</span>,
    source_payable_entry_id: (data: string): ReactElement => <span className="font-mono text-[10px]">{data.substring(0, 8)}</span>,
    target_payable_entry_id: (data: string): ReactElement => <span className="font-mono text-[10px]">{data.substring(0, 8)}</span>,
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 space-y-6">
          ...
          <div className="rounded-2xl border border-blue-500/30 bg-blue-500/5 p-5 space-y-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-blue-500/20 pb-3 gap-2">
              ...
              <div className="rounded-xl bg-blue-600 px-4 py-2 text-white font-extrabold text-sm">
                {pageDict.availableDebit}: {fmtMoney(selectedSourceEntry.remaining_minor, selectedSourceEntry.currency)}
              </div>
            </div>
                  ...
                  <table className="w-full text-start text-xs">
                    <thead className="bg-[var(--background)] text-[var(--text-muted)] font-bold">
                      <tr>
                        ...
                        <th className="p-3 text-end">{pageDict.originalCredit}</th>
                        <th className="p-3 text-end">{pageDict.remainingOpenCredit}</th>
                      ...
                      {openTargetCredits.map((target, idx) => {
                        ...
                        return (
                          <tr key={target.id} className="hover:bg-[var(--background)]/50">
                            ...
                            <td className="p-3">{target.description || pageDict.supplierBillCreditFallback}</td>
        ...
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-xs space-y-4">
          ...
          <div className="overflow-hidden rounded-xl border border-[var(--border)]">
            <ServerDataTable
              ajaxUrl="/purchasing/payable-settlements/data"
              columns={columns}
              filters={{ supplier_id: selectedSupplierId }}
              locale={locale}
              order={[[0, 'desc']]}
              pageLength={25}
              reloadToken={reloadToken}
              slots={slots}
              tableId="payable-settlements-table"
            />
          </div>
        </div>
