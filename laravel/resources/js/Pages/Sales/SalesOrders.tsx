...
export default function SalesOrdersIndex({ locale, salesOrders, customers, currencies, products, filters }: SalesOrdersProps) {
  ...
  const handleAction = (orderId: string, action: 'submit' | 'confirm' | 'cancel') => {
    let confirmMsg = '';
    if (action === 'submit') confirmMsg = isAr ? 'هل أنت تأكد من تقديم أمر البيع هذا؟' : 'Submit this Sales Order?';
    if (action === 'confirm') confirmMsg = isAr ? 'هل أنت تأكد من تأكيد أمر البيع وتخصيص الرقم؟' : 'Confirm this Sales Order and allocate order number?';
    if (action === 'cancel') confirmMsg = isAr ? 'هل أنت تأكد من إلغاء أمر البيع هذا؟' : 'Cancel this Sales Order?';
  ...
  return (
    <AppLayout active="sales-orders.index">
      <Head title={isAr ? 'أوامر البيع' : 'Sales Orders'} />

      <PageHeader
        title={isAr ? 'أوامر البيع' : 'Sales Orders'}
        description={isAr ? 'إدارة أوامر البيع والتزامات العملاء' : 'Manage customer sales orders and commitments'}
        actions={
          <button
            type="button"
          ...
          >
            ...
            <span>{isAr ? 'إنشاء أمر بيع' : 'Create Sales Order'}</span>
          </button>
        }
      />
      ...
      <Card className="p-6">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative flex-1 max-w-md">
            <input
              type="text"
              placeholder={isAr ? 'بحث بالرقم، المرجع، أو اسم العميل...' : 'Search number, reference, or customer...'}
        ...
        {salesOrders.data.length === 0 ? (
          <EmptyState
            title={isAr ? 'لا توجد أوامر بيع' : 'No Sales Orders found'}
            description={isAr ? 'ابدأ بإنشاء أول أمر بيع للعملاء' : 'Get started by creating your first sales order'}
          />
        ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-xs overflow-y-auto">
          <div className="w-full max-w-3xl rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-2xl my-8">
            <h3 className="text-base font-bold text-[var(--text-primary)] mb-4">
              {editingOrder
                ? isAr ? 'تعديل أمر البيع' : 'Edit Sales Order'
                : isAr ? 'إنشاء أمر بيع جديد' : 'Create Sales Order'}
            </h3>
              ...
              <div className="pt-4 border-t border-[var(--border)]">
                <div className="flex items-center justify-between mb-3">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-[var(--text-primary)]">
                    {isAr ? 'بنود أمر البيع' : 'Order Lines'}
                  </h4>
                  <button
                    type="button"
                    onClick={addLineItem}
