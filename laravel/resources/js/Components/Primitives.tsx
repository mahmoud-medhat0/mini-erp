...
export function Button({
  ...
  title?: string;
  'aria-label'?: string;
  /** HTML `form` attribute — submits a `<form>` this button isn't nested inside (e.g. a modal footer button). */
  form?: string;
}) {
...
/**
 * Shared modal shell: sticky header, internally-scrollable body capped to
 * the viewport (`100dvh`, keyboard-aware on mobile), optional sticky footer.
 * Avoids the `items-center` + backdrop `overflow-y-auto` combo, which makes
 * the top of tall content unreachable by scroll once it exceeds the viewport.
 */
export function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-xs">
      <div className={`flex w-full ${MODAL_SIZE_CLASSES[size]} max-h-[calc(100dvh-2rem)] flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface)] shadow-2xl`}>
        <div className="flex shrink-0 items-center justify-between gap-3 border-b border-[var(--border)] px-6 py-4">
          ...
          {hideCloseButton ? null : (
            <button
              ...
              className="shrink-0 rounded-lg p-1 text-lg font-bold leading-none text-[var(--text-secondary)] hover:bg-[var(--background)] hover:text-[var(--text-primary)] cursor-pointer"
            >
              &times;
            </button>
          )}
