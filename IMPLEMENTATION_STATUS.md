...
| Phase 7 Slice 2 Tax Code and Tax Rate Foundation | COMPLETE | `tax_codes` and `tax_rates` tables/models created with PostgreSQL check constraints `chk_tc_tax_type`, `chk_tc_calc_mode`, `chk_tc_rec_mode`, `chk_tr_rate_bps`, `TaxMasterDataService` (CRUD, system/in-use protection, audit), `TaxCalculationService` (effective rate resolution, integer minor-unit math for exclusive, inclusive, and exempt modes), `TaxCodeController`, `TaxRateController`, web routes, Inertia React pages (`Codes/Index.tsx`, `Codes/Create.tsx`, `Codes/Edit.tsx`, `Rates/Index.tsx`), EN/AR dictionary keys, navigation in `AppLayout.tsx`, `TaxCodeSeeder`, and 7/7 passing feature tests (`Phase7Slice2TaxFoundationTest`, 38 assertions). |
| Phase 7 Tax / VAT | IN PROGRESS | Phase 7 Slices 1 and 2 are 100% complete and locally verified. Slices 3-7 prompts prepared. |
